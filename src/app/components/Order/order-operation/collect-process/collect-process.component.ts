...
@Component({
  ...
  styleUrl: './collect-process.component.css'
})
export class CollectProcessComponent {
  ...
  ngOnInit(): void {
    this.activatedRoute.params.subscribe((params) => {
      ...
      if (params["innerNumber"]) {
        this.innerNumber = params["innerNumber"]
      }

      if (this.applicationCode && this.processCode && this.innerNumber) {
        if (this.applicationCode.toLocaleLowerCase() == "invoice") {
          this.toasterService.info("Fatura Ürünleri Getiriliyor")
          this.getCollectableProductsOfProcess();
        } else if (this.applicationCode.toLocaleLowerCase() == "order") {
          this.toasterService.info("Sipariş Ürünleri Getiriliyor")
          this.getCollectableProductsOfProcess();
        } else if (this.applicationCode.toLocaleLowerCase() == "proposal") {
          this.toasterService.info("Teklif Ürünleri Getiriliyor")
          this.getCollectableProductsOfProcess();
        } else if (this.applicationCode.toLocaleLowerCase() == "transfer") {
          this.toasterService.info("Transfer Ürünleri Getiriliyor")
          this.getCollectableProductsOfProcess();
        }
  ...
  //YENİ KODLAR-------------------------------------------- 06.10
  async getCollectableProductsOfProcess() {
  }
  async getCollectedProdutsOfProducts() {
