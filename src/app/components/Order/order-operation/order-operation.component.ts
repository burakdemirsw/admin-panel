...
export class OrderOperationComponent implements OnInit {
  ...
  async getOrderDetail() {
    this.orderDetail = await this.orderService.getOrderDetail(
      this.currentOrderNo
    );
    // console.log(this.orderDetail)
    // this.toasterService.info(this.orderDetail.taxTypeCode.toString());
  }
  ...
  async ngOnInit() {
    ...
    await this.activatedRoute.params.subscribe(async (params) => {
      ...
      if (orderNumberType === "BP") {
        ...
        var response = await this.orderService.getOrderDetail(
          params["orderNumber"]
        );
  ...
  async deleteMissingProduct(orderNumber: string, barcode: string) {
    ...
    if (confirmDelete) {
      var response = await this.orderService.deleteMissingProduct(
        this.currentOrderNo,
        barcode
      );
      if (response) {
  ...
  async showDialog2() {
    ...
    var response = await this.orderService.getInvoicesOfCustomer(
      this.currentOrderNo
    );
    ...
    if (this.isLocked) {
      this.setTopProduct(
        this.productsToCollect.find(
          (p) => p.lineId == this.lockedSetProduct.lineId
        ).setProducts
      );
    } else {
        ...
        if (_foundedProduct == undefined || _foundedProduct.quantity == 0) {
          if (this.lastCollectedProduct == null) {
            ...
            if (!this.isLocked) {
              ...
              var foundedProduct = this.productsToCollect
                .find((p) => p.lineId == this.lockedSetProduct.lineId)
                .setProducts.find(
                  (p) =>
                    p.barcode == this.lastCollectedProduct.barcode &&
                    p.itemCode == this.lastCollectedProduct.itemCode &&
    ...
    if (matchingProduct) {
      ...
      if (index !== -1) {
        if (this.productsToCollect.length - 1 >= index + 1) {
          ...
          this.checkForm
            .get("shelfNo")
            .setValue(this.productsToCollect[0].shelfNo);
        }
  ...
  async checkClientOrderByOrderNumber(orderNumber: string) {
    var response = await this.orderService.checkClientOrderByOrderNumber(
      orderNumber
    );
    if (response) {
      this.cancelStatus = true;
      this.toasterService.error(
        "Bu Sipariş İptal Edilmiştir Lütfen Ürünleri Yerine Yerleştiriniz"
      );
    }
  }
  warehosueIds: number[] = [61, 17, 30, 31, 55];
  async collectAndPack_WS(products: ProductOfOrder[]) {

    //eğer adminse seçicek değilse var olanı direkt göndercek
    if (!this.selectedInvoiceType && !this.warehosueIds.includes(this.userInfo.userId)) {
    ...
    const response: CountConfirmData[] =
      await this.orderService.getInventoryFromOrderNumber(this.currentOrderNo);
    if (response.length > 0) {
      ...
      if (response2) {
        ...
        var taxtypecode = this.selectedInvoiceType?.key == undefined ? this.orderDetail.taxTypeCode : this.selectedInvoiceType?.key;
        var _response = await this.orderService.collectAndPack(
          products,
          this.currentOrderNo,
          taxtypecode
        );
    ...
    if (orderType === "BP" || orderType === "WS" || orderType === "MIS") {
      ...
      if (confirmation) {
        ...
        if (orderType === "WS" || orderType === "MIS") {
          ...
          //EĞER ADMIN İSE SOR
          if (!this.warehosueIds.includes(this.userInfo.userId)) {
            this.showDialog();
            //this.spinnerService.hide();
            return;
          }
          //EĞER ADMIN DEĞİLSE DİREKT AT
          else {
            await this.collectAndPack_WS(this.collectedProducts);
          }

        } else {
          this.orderService.collectAndPack(
            products,
            this.currentOrderNo,
            null,
  ...
  async setFormValues(barcode: string): Promise<CountProduct> {
    try {
      if (barcode.includes("http") || this.generalService.isGuid(barcode)) {
        ...
        var product: CountProduct = new CountProduct(
          result[3],
          currentShelfNo,
          result[2],
          Number(result[1])
        ...
        if (
          this.checkForm.get("quantity").value == null ||
          this.checkForm.get("quantity").value == 1
        ) {
          this.checkForm.get("quantity").setValue(result[1]);
        }
  ...
  async onSubmit(productModel: CountProduct): Promise<any> {
    ...
    if (!this.checkForm.valid) {
      var updated_product = await this.setFormValues(productModel.barcode);
      ...
      if (
        ...
        if (
          (this.currentOrderNo.split("-")[1] === "WS" ||
            this.currentOrderNo.includes("MIS-")) &&
    ...
    if (
      ...
      if (this.checkForm.valid) {
        ...
        if (this.isLocked && this.lockedSetProduct) {
          ...
          if (
            this.lockedSetProduct.setProducts.some(
              (p) => p.quantity > 0 && p.barcode == productModel.barcode
            )
          ) {
            var sp = this.lockedSetProduct.setProducts.find(
              (p) => p.quantity > 0 && p.barcode == productModel.barcode
            );
            ...
            var result: string[] =
              await this.productService.countProductByBarcode3(
            ...
            if (foundProduct.quantity - productModel.quantity >= 0) {
              var _setRequest: ZTMSG_CountedSetProduct =
                new ZTMSG_CountedSetProduct();
              ...
              if (set_response) {
                ...
                var lp = this.productsToCollect.find(
                  (p) => p.lineId == this.lockedSetProduct.lineId
                );
        ...
        if (true) {
          ...
          if (
            ...
            if (foundProduct.quantity - productModel.quantity >= 0) {
              var lineId = this.productsToCollect.find(
                (p) => p.barcode == productModel.barcode
              ).lineId;
              if (!lineId) {
    ...
    //transfer-------------------------------------------------------------------- WT
      ...
      this.currentOrderNo.split("-")[1] === "WT" ||
      this.isBPTransferForm === true
    ) {
      ...
      if (this.checkForm.valid) {
        ...
        if (foundModel2 != null && foundModel2 != undefined) {
          ...
          if (foundProduct.quantity - productModel.quantity >= 0) {
            var lineId = this.productsToCollect.find(
              (p) =>
                p.barcode == productModel.barcode &&
                p.shelfNo == productModel.shelfNo
            ).lineId;
            if (!lineId) {
              this.toasterService.error("lineId bulunamadı");
            }
  ...
  urr: string =
    "https://lh3.googleusercontent.com/d/1H2pjyH0zqSbZMgw5pn1zU0DlbrgDlL5K";
  ...
  clearBarcodeAndQuantity() {
    if (
      ...
      this.currentOrderNo.includes("WT")
    ) {
      if (this.lastCollectedProduct) {
        this.checkForm
          .get("shelfNo")
          .setValue(this.lastCollectedProduct.shelfNo);
      }
      this.checkForm.get("batchCode").setValue(null);
    }
  ...
  async deleteOrderProduct(
    ...
    if (confirmDelete) {
      ...
      if (product.setProducts.length > 1) {
        var delete_set_response =
          await this.warehouseService.deleteCountedSetProductByOrder(
            orderNo,
            product.itemCode
          );
    ...
    if (this.lockedSetProduct?.lineId != setProduct.lineId) {
      if (setProduct.setProducts.length > 0) {
        ...
        if (setProduct.setProducts.some((p) => p.quantity > 0)) {
          ...
          this.stickToTop(
            setProduct.setProducts.find((p) => p.quantity > 0),
            true
          );
  ...
  async deleteSetProduct(product: ProductOfOrder) {
    var response = await this.warehouseService.deleteSetCount(
      this.orderNo,
      product.barcode
    );
    await this.getAllProducts(this.orderNo, "WS");
